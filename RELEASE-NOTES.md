...
## v6.1.1 (fork: Conansgithub/superpowers)

Fork sync — merges upstream `obra/superpowers` v6.1.1 into our fork while
preserving the local `spec-driven-development` gate engine and spec-as-AI-memory
workflow. The upstream Codex marketplace/package fixes and `hooks: {}` behavior
are adopted; the fork-specific spec gate remains available under
`skills/spec-driven-development/gate`.
